  ...
  before: {
    ...
    create: [
     async ( hook ) => {
       ...
       let registry = await hook.app.service ( `${REGISTRY_PATH}` ).get (  subscriberId );
  ...
  after: {
    ...
    create: [
       hook  => {
        const { params  , payload, data } = hook;
         if(hook.result) {
           hook.app.service ( `${USERS_PATH}` ).find ( { query : { id : data.subscriber.id } } )
             .then ( ( { data } ) => {
               hook.app.service ( `${USERS_PATH}` ).patch ( null ,{
